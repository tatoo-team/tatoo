...
<% } %>
 ...
 %>
    ...
    public void reduce(<%=productionEnum.getSimpleName()%> production) {
      SemanticStack stack=this.stack;
      switch(production) { <%
          ...
          <% //if it's not a star/optional production
                    if (ebnfType==null) {%>
                    reduce_<%=production.getId() %>(stack);<%
                    } else {          
                                  switch(ebnfType) {
                                     case OPTIONAL_EMPTY:
                                       if (type!=null && !type.isVoid()) {
    ...
    <% for(ProductionDecl production:productions) { 
         ...
         EBNFSyntheticType ebnfType=ebnfSupport.getEBNFTypeMap().get(production);
         if (ebnfType==null) { %>
         <% if (isOneTypeParametrized(production.getRight(),variableTypeMap)) {%>
         @SuppressWarnings("unchecked")
         <% } %>
         private void reduce_<%=production.getId() %>(SemanticStack stack) {<%
